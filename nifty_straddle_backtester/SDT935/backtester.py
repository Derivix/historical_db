...
from utils.premium_selection import _last_price, _premium_close_to, _premium_on_atm
from utils.datatype import SDBreakout935, _Leg
...
class SDBreakoutBacktester:
    ...
    def __init__(self, repo, symbol: str, start_date: str, end_date: str, cfg: SDBreakout935):
        ...
        self.strike_search_steps = self.cfg.strike_search_steps
    ...
    def _marked_portfolio_pnl(self, legs: list[_Leg], when: dt.datetime) -> float | None:
        """Return gross P&L using exits for closed legs and live prices otherwise."""
        pnl = 0.0
        for leg in legs:
            if leg.exit_price is not None:
                pnl += leg.gross_pnl
                continue
            price = self._price(leg, when)
            if price is None:
                return None
            pnl += (leg.entry_price - price) * leg.quantity
        return pnl
    ...
    def simulate_day(self, day: dt.date, trigger_pct: float) -> dict | None:
        ...
        monitor_ce = _premium_on_atm(self.repo, self._option_cache, self.underlying_id, day, expiry, "CE", entry_dt, entry_spot, self.strike_step, self.cfg, atm_offset=self.cfg.atm_offset)
        monitor_pe = _premium_on_atm(self.repo, self._option_cache, self.underlying_id, day, expiry, "PE", entry_dt, entry_spot, self.strike_step, self.cfg, atm_offset=self.cfg.atm_offset)
        ...
        reversal_monitor_ce: _Leg | None = None
        reversal_monitor_pe: _Leg | None = None
        reversal_trigger: float | None = None
        reversal_start_time: dt.datetime | None = None
        reversal_start_ce_px: float | None = None
        reversal_start_pe_px: float | None = None
        reversal_start_combined_px: float | None = None
        reversal_trigger_time: dt.datetime | None = None
        reversal_trigger_ce_px: float | None = None
        reversal_trigger_pe_px: float | None = None
        reversal_trigger_combined_px: float | None = None
        ...
        # State sequence: original monitoring straddle -> new-ATM reversal
        # straddle -> reversal trigger -> replacement short.
        # Neither monitoring straddle is itself traded.
        for _, row in session[session["ts"] > entry_dt].iterrows():
            when, spot_px = row["ts"], float(row["close"])
            if cancelled_reason == "none":
                ...
                if current_value >= up_trigger:
                    ...
                    if ce_px is None or pe_px is None:
                        continue
                    # Close only the original leg that drove the monitoring
                    # straddle.  The other original short remains live.
                    directional_driver = "CE" if ce_change_pct >= pe_change_pct else "PE"
                    if directional_driver == "CE":
                        ce.close(when, ce_px)
                    else:
                        pe.close(when, pe_px)
                    cancelled_reason = f"straddle_up_{trigger_pct:g}pct_{directional_driver}_driver_close_{directional_driver.lower()}"
                else:
                    continue

                # Establish a fresh ATM straddle at the first trigger time.
                # Its later expansion is the reversal signal.
                reversal_monitor_ce = _premium_on_atm(self.repo, self._option_cache, self.underlying_id, day, expiry, "CE", when, spot_px, self.strike_step, self.cfg)
                reversal_monitor_pe = _premium_on_atm(self.repo, self._option_cache, self.underlying_id, day, expiry, "PE", when, spot_px, self.strike_step, self.cfg)
                if reversal_monitor_ce is None or reversal_monitor_pe is None:
                    exit_reason, exit_dt = "reversal_monitor_unavailable", when
                    break
                reversal_start_time = when
                reversal_start_ce_px = reversal_monitor_ce.entry_price
                reversal_start_pe_px = reversal_monitor_pe.entry_price
                reversal_start_combined_px = reversal_start_ce_px + reversal_start_pe_px
                reversal_trigger = reversal_start_combined_px * (1 + self.cfg.reverse_trend_pct / 100)
                continue
            ...
            if replacement is not None:
                ...
                if replacement_px is None:
                    continue
                portfolio_gross = self._marked_portfolio_pnl(all_legs, when)
                
                # Target Profit Exit: replacement leg premium falls by wait_and_trade_pct (e.g. -15%)
                if self.cfg.wait_and_trade_pct < 0:
                    target_profit_px = replacement.entry_price * (1 + self.cfg.wait_and_trade_pct / 100)
                    if replacement_px <= target_profit_px:
                        replacement.close(when, replacement_px)
                        exit_reason, exit_dt = f"replacement_target_profit_{abs(self.cfg.wait_and_trade_pct):g}pct", when
                        break

                if replacement_px >= replacement.entry_price * (1 + self.cfg.adjustment_stop_pct / 100):
                    replacement.close(when, replacement_px); exit_reason, exit_dt = "adjustment_stop_loss_25pct", when; break
                if portfolio_gross is not None and portfolio_gross <= -self.cfg.portfolio_loss_limit:
                    exit_reason, exit_dt = "combined_stop_loss", when; break
                continue

            # Monitor the current-ATM reversal straddle until its configured expansion.
            assert reversal_monitor_ce is not None and reversal_monitor_pe is not None and reversal_trigger is not None
            reversal_ce_px = self._price(reversal_monitor_ce, when)
            reversal_pe_px = self._price(reversal_monitor_pe, when)
            if reversal_ce_px is None or reversal_pe_px is None:
                continue
            reversal_value = reversal_ce_px + reversal_pe_px
            if reversal_value < reversal_trigger:
                continue

            reversal_trigger_time = when
            reversal_trigger_ce_px = reversal_ce_px
            reversal_trigger_pe_px = reversal_pe_px
            reversal_trigger_combined_px = reversal_value
            ce_change_pct = (reversal_ce_px / reversal_monitor_ce.entry_price - 1) * 100
            pe_change_pct = (reversal_pe_px / reversal_monitor_pe.entry_price - 1) * 100
            reversal_driver = "CE" if ce_change_pct >= pe_change_pct else "PE"
            replacement_type = "PE" if reversal_driver == "CE" else "CE"

            replacement = _premium_close_to(self.repo, self._option_cache, self.underlying_id, day, expiry, replacement_type, when, spot_px, self.cfg.adjustment_premium, self.strike_step, self.cfg)
            if replacement is None:
                continue
            all_legs.append(replacement)
            cancelled_reason += f"_reversal_{self.cfg.reverse_trend_pct:g}pct_{reversal_driver}_driver_sell_{replacement_type.lower()}"
        else:
        ...
        return {
            ...
            "adjustment_reason": cancelled_reason,
            "directional_driver": directional_driver,
            "reversal_start_time": reversal_start_time,
            "reversal_start_ce_px": reversal_start_ce_px,
            "reversal_start_pe_px": reversal_start_pe_px,
            "reversal_start_combined_px": reversal_start_combined_px,
            "reversal_trigger_time": reversal_trigger_time,
            "reversal_trigger_ce_px": reversal_trigger_ce_px,
            "reversal_trigger_pe_px": reversal_trigger_pe_px,
            "reversal_trigger_combined_px": reversal_trigger_combined_px,
            "reversal_monitor_ce_ticker": reversal_monitor_ce.ticker if reversal_monitor_ce else "",
            "reversal_monitor_pe_ticker": reversal_monitor_pe.ticker if reversal_monitor_pe else "",
            "reversal_straddle_trigger": reversal_trigger,
            "wait_and_trade_target_price": replacement.entry_price * (1 + self.cfg.wait_and_trade_pct / 100) if (replacement and self.cfg.wait_and_trade_pct < 0) else None,
            "reversal_driver": reversal_driver,
            ...
            "adjustment_stop": replacement.entry_price * (1 + self.cfg.adjustment_stop_pct / 100) if replacement else None,
    ...
    def run_and_export(
        ...
        for day_number, day in enumerate(days, start=1):
            try:
                for trigger_number, trigger_pct in enumerate(trigger_values, start=1):
                    if progress_callback is not None:
                        progress_callback(day_number, len(days), day, trigger_number, len(trigger_values))
        ...
        for trigger_pct in trigger_values:
            ...
            parameter_rows.append(base_parameters | {
                ...
                "adjustments": int((subset["replacement_type"] != "").sum()) if not subset.empty else 0,
            })
