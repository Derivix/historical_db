...
# from utils.datatype import SDBreakoutConfig, _Leg
# from utils.premium_selection import _premium_close_to, _premium_on_atm
...
#             return None
#         ce = _premium_close_to(day, expiry, "CE", entry_dt, entry_spot, self.cfg.initial_premium)
#         pe = _premium_close_to(day, expiry, "PE", entry_dt, entry_spot, self.cfg.initial_premium)
...
from engine.costs import compute_leg_cost
from utils.premium_selection import _premium_close_to, _premium_on_atm
from utils.datatype import SDBreakout935, _Leg, _last_price, _max_drawdown
...
class SDBreakoutBacktester:
    ...
    def simulate_day(self, day: dt.date, trigger_pct: float) -> dict | None:
        ...
        monitor_ce = _premium_on_atm(self.repo, self._option_cache, self.underlying_id, day, expiry, "CE", entry_dt, entry_spot, self.cfg.trigger_straddle_atm, self.cfg.strike_step, self.cfg)
        monitor_pe = _premium_on_atm(self.repo, self._option_cache, self.underlying_id, day, expiry, "PE", entry_dt, entry_spot, self.cfg.trigger_straddle_atm, self.cfg.strike_step, self.cfg)
        ...
        for _, row in session[session["ts"] > entry_dt].iterrows():
            ...
            if replacement is None:
                ...
                driver, cancelled_type = (
                    ("CE", "CE") if ce_change_pct >= pe_change_pct else ("PE", "PE")
                )
                directional_driver = driver
                ...
                if current_value >= up_trigger:
                    ...
                    if driver == "CE":
                        ...
                        pe.close(when, pe_px)
                    cancelled_reason = f"straddle_up_{trigger_pct:g}pct_{driver}_driver_sell_{cancelled_type.lower()}"
                else:
                    continue

                reversal_monitor_ce = _premium_on_atm(self.repo, self._option_cache, self.underlying_id, day, expiry, "CE", when, spot_px, self.cfg.strike_step, self.cfg)
                reversal_monitor_pe = _premium_on_atm(self.repo, self._option_cache, self.underlying_id, day, expiry, "PE", when, spot_px, self.cfg.strike_step, self.cfg)

                if reversal_monitor_ce is None or reversal_monitor_pe is None:
                    continue

                reversal_straddle = reversal_monitor_ce.entry_price + reversal_monitor_pe.entry_price

                up_trigger_reversal = reversal_straddle * (1 + self.cfg.reverse_trend_pct / 100)
